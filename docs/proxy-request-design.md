...
│  ┌──────────────┐              ┌──────────────┐                         │
│  │   用户A     │──────────────▶│   用户B      │                         │
│  │ (Requestor)  │  飞书推送     │  (Owner)     │                         │
│  └──────────────┘              └──────────────┘                         │
└─────────────────────────────────────────────────────────────────────────────────────┘

说明：
1. 飞书 WebSocket (现有) - 接收所有用户的文本消息
2. 飞书长连接 (新增) - 接收卡片交互回调（通过飞书 SDK 建立）
3. 两条连接都连到同一个飞书机器人（同一个 APP_ID）
4. HTTP API - 发送和更新审批卡片（长连接模式下卡片不支持 WebSocket）
```
